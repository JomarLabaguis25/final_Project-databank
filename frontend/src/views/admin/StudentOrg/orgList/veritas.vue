...
<div style="font-family: Arial, sans-serif; margin: 20px;">
        ...
        <h2 style="text-align: center;">The Veritas' Event Calendar</h2>
    ...
    <tbody v-for="info in info" :key="info.id">
        <tr>
            ...
            <td style="padding: 12px; border: 1px solid #ddd;">
              <button class="btn btn-primary" @click="confirmUpdate(info)">Update</button>
...
<div v-if="selectedInfo">
    ...
    <form @submit.prevent="updateTask">
      <label>Date:</label>
      <input v-model="selectedInfo.eventDate" required>

      <label>Name:</label>
      <input v-model="selectedInfo.eventName" required>

      <label>Location:</label>
      <input v-model="selectedInfo.eventLoc" required>

      <label>Description:</label>
      <textarea v-model="selectedInfo.eventDesc" required></textarea>

      <label>Status:</label>
      <select v-model="selectedInfo.Status" required>
      ...
      <input v-model="selectedInfo.OrgName" required>
...
export default {
  data() {
    return {
      ...
      info: [],
      showDetails: {} ,
      info: [], // Your data array
      selectedInfo: null, // To store the information being updated// Assuming you have showDetails property
    };
  },
  created() {
    this.getOrgData();
...
confirmUpdate(info) {
    ...
    if (window.confirm('Are you sure you want to update this data?')) {
      // User clicked OK, set selectedInfo for update
      this.selectedInfo = { ...info }; // Create a copy of the selected info
    } else {
  ...
  // For example, update the 'info' array with the new data
  console.log('Updating task with selectedInfo:', this.selectedInfo);

  const index = this.info.findIndex(item => item.id === this.selectedInfo.id);
  console.log('Index found:', index);

  if (index !== -1) {
    // Directly update the property values
    Object.assign(this.info[index], this.selectedInfo);
    console.log('Updated info array:', this.info);
  } else {
    console.error(`Task with ID ${this.selectedInfo.id} not found in the info array.`);
  }

  // Optionally, you can make an API call to update the data on the server
  // Replace the following line with your actual API call
  console.log(`Updating data with ID ${this.selectedInfo.id} on the server...`);

  // Provide feedback or update your data source accordingly
  console.log(`Data with ID ${this.selectedInfo.id} updated successfully.`);

  // Reset selectedInfo to null to hide the update form
  this.selectedInfo = null;
    ...
    async deleteTask() {
      try {
        ...
        const response = await this.$axios.delete('/api/deleteTask', {
          data: { id: this.taskId },
        });

        // Check if the deletion was successful
        if (response.status === 200) {
          // Remove the task from the client-side data array (assuming 'info' is your data array)
          const index = this.info.findIndex(info => info.id === this.taskId);
          if (index !== -1) {
          ...
          console.error(`Failed to delete task with ID ${this.taskId}.`);
        }
      } catch (error) {
